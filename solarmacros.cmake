set(CMAKE_INSTALL_PREFIX $ENV{BCOMDEVROOT} CACHE INTERNAL "")
####################################################
...
macro (defineTargets EXEORLIBRARY FILES_TO_COPY)
	...
	if ("${EXEORLIBRARY}" STREQUAL "library") # only for libraries
		...
		install (TARGETS ${PROJECT_NAME} DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER}/lib/${PROJECT_ARCH}/shared/${BUILDCONFIG})
		if (WIN32)
			if (MSVC)
				install(FILES $<TARGET_PDB_FILE:${PROJECT_NAME}> DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER}/lib/${PROJECT_ARCH}/shared/${BUILDCONFIG} OPTIONAL)
			endif(MSVC)
		endif(WIN32)
		# install interfaces
		install (DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/interfaces/"        
		         DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER}/interfaces)
		install (FILES "${CMAKE_CURRENT_SOURCE_DIR}/bcom-${PROJECT_NAME}.pc.in" DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER} RENAME "bcom-${PROJECT_NAME}.pc")
		install (FILES "${CMAKE_CURRENT_SOURCE_DIR}/packagedependencies-cmake.txt" DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER})
		install (FILES "${CMAKE_CURRENT_SOURCE_DIR}/packagedependencies.txt" DESTINATION $ENV{BCOMDEVROOT}/${PROJECT_NAME}/${VERSION_NUMBER})
...
macro (setup)
	IF(WIN32) # Check if we are on Windows
	  if(MSVC) # Check if we are using the Visual Studio compiler
	  	...
	  	add_definitions (-DBOOST_ALL_DYN_LINK -DBOOST_AUTO_LINK_NOMANGLE -DBOOST_ALL_NO_LIB -DBOOST_LOG_DYN_LINK -DXPCFVERSION=\"\")
