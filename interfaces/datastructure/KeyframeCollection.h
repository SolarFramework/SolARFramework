...
public:
	...
	FrameworkReturnCode addKeyframe(const SRef<datastructure::Keyframe> keyframe);

	/// @brief This method allow to add a frame to the key frame manager component
	/// @param[in] frame the frame to add to the set of persistent keyframes
	/// @return FrameworkReturnCode::_SUCCESS_ if the addition succeed, else FrameworkReturnCode::_ERROR.
	FrameworkReturnCode addKeyframe(const datastructure::Keyframe & keyframe);
	...
	FrameworkReturnCode getKeyframe(const uint32_t id, SRef<datastructure::Keyframe> & keyframe) const;
	...
	/// @return FrameworkReturnCode::_SUCCESS_ if succeed, else FrameworkReturnCode::_ERROR.
	FrameworkReturnCode getKeyframes(const std::vector<uint32_t> & ids, std::vector<SRef<datastructure::Keyframe>>& keyframes) const;

	/// @brief This method allows to get all keyframes
	/// @param[out] the set of keyframes
	/// @return FrameworkReturnCode::_SUCCESS_ if succeed, else FrameworkReturnCode::_ERROR.
	FrameworkReturnCode getAllKeyframes(std::vector<SRef<datastructure::Keyframe>>& keyframes) const;
